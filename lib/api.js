...
// Only system_prompt is actually persisted by the backend (model is
// env/code-controlled, name/description are hardcoded in tribe_app/
// pfc's agent.py) — see routers/admin.py's module docstring on the
// backend. Sending other fields here would silently do nothing, so
// this only ever sends the one field the API honors.
export function updateAgentSystemPrompt(agentId, systemPrompt) {
  return request(`/admin/agents/${agentId}`, {
    method: "PUT",
...
export function resetAgentSystemPrompt(agentId) {
  return request(`/admin/agents/${agentId}/system-prompt`, { method: "DELETE" });
}
...
// --- Test chat ---
// sessionId is optional on the first message (the backend mints one and
// returns it) but must be passed back on every message after that, or
// each call lands in a brand-new session with no memory of the last one
// — multi-turn flows like the booking confirmation gate depend on this.
export function testChat(agentId, message, sessionId) {
  return request(`/admin/agents/${agentId}/test-chat`, {
    method: "POST",
